<template>
	<div class="flex h-full w-full flex-col">
		...
		<div class="flex flex-1 flex-col gap-3 rounded bg-white text-black sm:p-10 max-sm:p-5 dark:bg-surface-600 dark:text-white">
			<template v-if="!data.loading && data.novel">
				<template v-if="data.novel.status === 'publish'">
									...
									<div class="flex gap-1">
										...
										<a v-if="data.novel.source === 'bilibili'"
										   ...
										   class="w-min whitespace-nowrap rounded-lg bg-slate-100 px-2 py-0 text-xs leading-6 text-slate-700 dark:bg-surface-500 dark:text-white">
											{{ t('来源') }}<i class="mr-1 ml-2 fa-brands fa-bilibili"></i>bilibili
										</a>
										<a v-if="data.novel.source === 'pixiv'"
										   ...
										   class="w-min whitespace-nowrap rounded-lg bg-slate-100 px-2 py-0 text-xs leading-6 text-slate-700 dark:bg-surface-500 dark:text-white">
											{{ t('来源') }}<i class="mr-1 ml-2 fa-brands fa-product-hunt"></i>pixiv
										</a>
									</div>
								</div>
					...
					<div class="bottom-0 left-0 z-50 hidden h-16 w-full bg-white/70 text-black backdrop-blur-sm max-sm:flex max-sm:fixed dark:bg-surface-700/70 max-sm:text-white">
						<div class="flex flex-1">
							<div class="flex flex-1 items-center justify-center">
								<Button class="h-full flex-1 rounded-none text-sm text-primary-500"
								        :label="t('书架')"
								        size="small" text
								        @click="toggleBookmark">
									<div v-if="!isBookmarked"
									     class="flex flex-1 flex-col justify-center text-center align-middle text-surface-600 dark:text-white">
										<div class="mb-1 text-lg fa-regular fa-book-bookmark"></div>
										{{ t('加入书架') }}
									</div>
									<div v-else
									     class="flex flex-1 flex-col justify-center text-center align-middle text-surface-600 dark:text-white">
								...
								<Button v-if="canTranslate(data.novel?.tags || [])"
								        class="h-full flex-1 rounded-none text-sm text-primary-500"
								        :label="t('翻译')" size="small"
								        text
								        @click="isShowTranslate = !isShowTranslate">
									<div class="flex flex-1 flex-col justify-center text-center align-middle text-surface-600 dark:text-white">
										<template v-if="isShowTranslate">
											<div class="mb-1 text-lg fa-regular fa-language"></div>
											{{ t('原&nbsp;文') }}
										</template>
										<template v-else>
											<div class="mb-1 text-lg fa-regular fa-language"></div>
							...
							<router-link v-if="currentReadChapter"
							             :to="{name:'chapter', params:{id:data.novel.id, cid:currentReadChapter.id}}"
							             class="h-full text-sm text-primary-500 w-[50vw]">
				...
				<template v-else>
					<div class="flex w-full gap-3 h-[calc(100vh-150px)] justify-center items-center">
						<div class="text-center text-lg">
							{{ t('由于版权方要求，该小说暂时无法阅读。') }}
							<br>
							{{ t('请前往源站阅读。') }}
							<template v-if="data.novel.source === 'bilibili'">
								{{ `https://www.bilibili.com/read/readlist/rl${data.novel.source_id}/` }}
							</template>
							<template v-if="data.novel.source === 'pixiv'">
								{{
									data.novel.ext_data?.oneshot ? `https://www.pixiv.net/novel/show.php?id=${data.novel.source_id}` : `https://www.pixiv.net/novel/series/${data.novel.source_id}`
								}}
							</template>
						</div>
					</div>
			...
			<template v-if="data.novel && data.novel.status === 'publish'">
				<div class="">
					<TabView>
...
onMounted(() => {
    ...
    if (data.novel && data.novel.status) {
